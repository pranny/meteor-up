...
exports.setup = function(config) {
  ...
  if(config.setupRedis) {
    taskList.copy('Copying Redis configuration', {
      src: path.resolve(TEMPLATES_DIR, 'redis-server.conf'),
      dest: '/opt/redis-server/redis-server.conf'
    });

    taskList.executeScript('Installing Redis Server', {
      script: path.resolve(SCRIPT_DIR, 'install-redis.sh')
    });
  }
